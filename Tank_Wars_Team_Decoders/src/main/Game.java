...
public class Game implements KeyListener {
	...
	// **Objects in Game
	protected Sprite Shot; // displays weapon 1 for player 1 in the top left
							// corner
	protected Sprite SecondShot; // displays weapon 2 for player 1 in the top
									// left corner
	protected Sprite Shot2; // displays weapon 1 for player 2 in the top right
							// corner
	protected Sprite SecondShot2; // displays weapon 2 for player 2 in the top
									// right corner
	...
	public boolean firstfall = true;// sees if the tank is falling for the first
									// time
	public boolean firstfall2 = true;// sees if the tank is falling for the
										// first time
	...
	// 1=turn based
	// 3=restart game
	public boolean GameSTATEchanged = true; // has GameSTATE been changed since
											// starting the game
	...
	// tankshoot>1 means nothing besides that a shot was not just fired. need to
	// fix this
	...
	public boolean TankCreated = false; // Gate to detect if a tank was randomly
										// placed onto the map or not
	/*** Holder and Counters ***/
	int a = 0;
	...
	/*******************************************************
	 * Added by Ryan K Selects one of the terrains at random
	 *******************************************************
	 */
	...
	// AHMET- Changed from String to int. Why whould you want to make these
	// String?
	int[] nums = { 1, 2, 3 };
	int ran = (int) (Math.random() * nums.length);
	int levelNumber = nums[ran];
	...
	public void loadImages() {
		...
		arrow = loadImage("images/newarrow2.png"); // Arrow indicating which
													// tank is moving
		...
		// The add frames need to have -1 duration for color changes to take
		// effect (the frames of the set color are added when game starts)
		...
		// Take the Image Frames and Create and Object(Sprite). Sprites have
		// Attributes for locating its position, movement, and State. See
		// Sprite.Java
	...
	public void run() {
		...
		try {
			...
			// is only called when restartgame is set to 1 to exit the
			// animationloop
	...
	public void animationLoop() {
		...
		while (restartgame < 1) {
			// run loop infinitely till user exits.
			// ***************************************************************
	...
	public void update(long elapsedTime) {
		...
		// To create gravity, there needs to be a decay on the velocity at a
		// constant rate.
		// The decay must be at the correct interval to create a natural
		// trajectory for the shot.
		...
		if (turn == 1)
			T1Weapon.GravityAndCollision(Tank2);
		if (turn == 2)
			T2Weapon.GravityAndCollision(Tank1);
	...
	public BufferedImage rotateImage(Image image, double angle) {
		...
		// Rotate the image
		BufferedImage rotated = new BufferedImage(100, 100,
				BufferedImage.TYPE_INT_ARGB);
	...
	public synchronized void draw(Graphics2D g) {
		if (credits == 0) {
			...
			DrawTerrain(g); // draws a flat surface for the tanks to land on.
							// Use this function to implement a dynamic terrain
							// generation

			if (TankCreated != true) // if no tank has been created, then create
										// one.
			{
				CreateTank(g);
				Tank1.setTurretAngleX(Math.round(Tank1.getTankSprite().getX()
						+ (int) (Tank1.getTankSprite().getWidth() * .5)));
				Tank1.setTurretAngleY(Math.round(Tank1.getTankSprite().getY()
						- (int) (Tank1.getTankSprite().getHeight())));
				Tank2.setTurretAngleX(Math.round(Tank2.getTankSprite().getX()
						+ (int) (Tank2.getTankSprite().getWidth() * .5)));
				Tank2.setTurretAngleY(Math.round(Tank2.getTankSprite().getY()
						- (int) (Tank2.getTankSprite().getHeight())));
				TankCreated = true;
			} // end tank creation
			...
			GroundCollision(Tank2.getTankSprite(), 2); // check if there is
														// ground underneath a
														// tank, if not let it
														// fall. This function
														// is crudely done.
			// creating a center of gravity for the objects and doing a better
			// check on when a tank should fall can be changed in this function.

			// Draw the tank's turret. 19 = 1/2 centering the position of the
			// turret on the tank. 3 = making the turret at the correct height
			// of the tank.
			float[] dash = { 25f, 25f };
			...
			if (Mode1 == 1 || Mode2 == 1) {
				...
				g.rotate(Tank.getAngle() * -0.018 + 1.625,
						Math.round(Tank.getTankSprite().getX())
								+ (int) (Tank.getTankSprite().getWidth() * .5),
						Math.round(Tank.getTankSprite().getY()));
				g.drawLine(Math.round(Tank.getTankSprite().getX())
						+ (int) (Tank.getTankSprite().getWidth() * .5),
						Math.round(Tank.getTankSprite().getY()),
						Math.round(Tank.getTankSprite().getX())
								+ (int) (Tank.getTankSprite().getWidth() * .5),
			...
			g.drawLine(Math.round(Tank1.getTankSprite().getX())
					+ (int) (Tank1.getTankSprite().getWidth() * .5),
					Math.round(Tank1.getTankSprite().getY()),
					Tank1.getTurretAngleX(), Tank1.getTurretAngleY());
			g.drawLine(Math.round(Tank2.getTankSprite().getX())
					+ (int) (Tank2.getTankSprite().getWidth() * .5),
			...
			if (nextTurn == true) {
				...
				if (turn == 1)
					g.drawString("Your turn!",
							Tank1.getTankSprite().getX() - 15, Tank1
									.getTankSprite().getY() - 40);
				else
					g.drawString("Your turn!",
							Tank2.getTankSprite().getX() - 15, Tank2
									.getTankSprite().getY() - 40);
				if (textVisibility > 0)
					textVisibility -= 15;
				else {
			...
			g.drawImage(
					rotateImage(Tank1.getTankSprite().getImage(),
							Tank1.getTankSlant()),
					Math.round(Tank1.getTankSprite().getX()),
					Math.round(Tank1.getTankSprite().getY()), null);

			// player 2 tank image
			g.drawImage(
					rotateImage(Tank2.getTankSprite().getImage(),
							Tank2.getTankSlant()),
					Math.round(Tank2.getTankSprite().getX()),
					Math.round(Tank2.getTankSprite().getY()), null);
			...
			} else if (Tank2.getWeapon2() == 2)// Can't display till image is
												// smaller.
			{
			...
			// checks if Player 1's health is 0, this could be made into a
			// function.
			checkHealth(Tank1.getHealth(), Tank1.getTankSprite(), g);
			checkHealth(Tank2.getHealth(), Tank2.getTankSprite(), g);

			// checks if bullet hit ground if it did creates hole and sets
			// state=0
			...
			// if player 1 fired checks to see what he fired and fires that
			// shot.
			if (tankshoot == 1) {
				T1Weapon.TankFire();
				tankshoot++;
			...
			// if the shot is currently in the air, update the image of the
			// shot.
		...
		// Wouldn't have worked without Zach K. Can't break out of while loops
		// but can break out of if loops.
		if (credits == 1 && Credits.creditsOn && Credits.count < Credits.loop) {// Draws
																				// credits
			Credits.draw(g);
		}
	...
	private void drawTutorial(Graphics g) {
		...
		/*
		 * if (turn == 1) { g.drawImage(blackarrowleft, 170, 75, null);
		 * g.drawImage(keyarrowup, 240, 70, null); g.drawImage(keyarrowdown,
		 * 290, 70, null); g.drawString("to change mode", 240, 140); } else if
		 * (turn == 2) { g.drawImage(blackarrowright, 580, 75, null);
		 * g.drawImage(keyarrowup, 520, 70, null); g.drawImage(keyarrowdown,
		 * 470, 70, null); g.drawString("to change mode", 470, 140); }
		 */
		int barX = 0;
		int barY = 0;
		int barXOffset = 50;
		...
		if (turn == 1) {
			...
			/*
			 * if (Mode2 == 0) g.drawString("to move!", barX+80, barY+55); if
			 * (Mode2 == 1) g.drawString("to aim!", barX+80, barY+55); if (Mode2
			 * == 2) g.drawString("to adjust power!", barX+80, barY+55); if
			 * (Mode2 == 3) {
			 * 
			 * }
			 */
	...
	private void checkHealth(int health, Sprite player, Graphics g) {
		if (health <= 0) {
			...
			if (explosionFrames < 150) {
				g.drawImage(BOOM.getImage(), Math.round(player.getX() - 14),
						Math.round(player.getY() - 75), null);
				explosionFrames++;
			}
			// Message that displays when game is over
			g.drawString("GAME OVER: PLAYER " + playerNumber + " WINS", 300,
					230);
			g.drawString("Press 'Space' to play again", 310, 250);
			turn = 3; // When turn is equal to 3, neither player can go, this
						// will prevent players from continuing to fire
			// Adds sounds after game
			if (playerNumber == 1) { // Player 1 wins
				String Ending[] = { "Winner1.wav", "1Winner2.wav",
						"toasty.wav", "salmonwin.wav", "herscowin.wav" };
				...
				// Player 2 Wins
				String Ending[] = { "Winner2.wav", "2Winner2.wav",
						"toasty.wav", "salmonwin.wav", "herscowin.wav" };
				int ran = (int) Math.floor((Math.random() * Ending.length));
				String EndingNumber = Ending[ran];
				if (EndingActive == false) {
	...
	/*
	 * private void fireShot(Sprite shot, Graphics g) { if(shot.getX() >
	 * 0)//inside the screen { if(topy[Math.round(shot.getX())] <
	 * Math.round(shot.getY()))//above the top of the terrain {
	 * CreateHole(shot); } } }
	 */
	/*
	 * private void CreateHole(Sprite shot) { soundShot.close(); //resets sound
	 * after being shot hitTest = 2;//says the object hit int
	 * counter3=Math.round(shot.getX())-14; //Left/Right Starting Square pos
	 * adjustment on hole int holderSin=0; int holderCos=1; double
	 * groundHolder=180; while (groundHolder <= 359)//creates hole starting at
	 * 180 degrees and going to 360 { holderSin =
	 * (int)(Math.floor((Math.sin(groundHolder/57.3))*5));//uses a length of 10
	 * for hole if (counter3>3) { topy[counter3] = topy[counter3]-
	 * holderSin;//reduces terrain }//depth of square //
	 * 
	 * //creates an unequal cirlce if (holderCos==3) { counter3++; holderCos=0;
	 * }
	 * 
	 * holderCos++; groundHolder++; }
	 * 
	 * }
	 */
	...
	/*
	 * ModeIndex Mode 0 is movement Mode 1 is Turret angle Mode 2 is Power Mode
	 * 3 is Weapon Selection
	 * 
	 * 
	 * WeaponIndex 0 = Normal 1 = Nuclear 2 = Hersco 3 = Joker
	 */
	...
	public void keyPressed(KeyEvent e) {
		...
		if (credits == 0 && titleScreenOpen == 0 && helpScreenOpen == 0) {
			...
			if (keyCode == KeyEvent.VK_U) {
				if (ControlsOverlayOpen == false) {// if controls overlay isnt
													// open
			...
			if (keyCode == KeyEvent.VK_SPACE) {
				// AHMET- USE /* */ for long comments
				/*
				 * Resets game This will reset the terrain, replenish both
				 * player's health and set turn to player 1. Set Y sets the
				 * tanks to start on the ground Therefore not falling and losing
				 * health
				 */
				music.s();
				String track = gameMusic;
				...
				// AHMET- Same fix
				/*
				 * if (levelNumber == "1") { ChangeLevel("2"); } else if
				 * (levelNumber == "2") ChangeLevel("3"); else if (levelNumber
				 * == "3") { ChangeLevel("1"); }
				 */
				ChangeLevel(levelNumber % 3 + 1);
				...
				Mode1 = 0; // Makes it so both tanks are back to default weapon
							// when restared
			...
			if (keyCode == KeyEvent.VK_Z) {
				...
				if (terrainChange == true) {
					Tank1.getTankSprite().setY(599);
					Tank2.getTankSprite().setY(599);
					// AHMET--REDUCED 10 Lines to 1
					/*
					 * if (levelNumber == "1") { ChangeLevel("2"); } else if
					 * (levelNumber == "2") ChangeLevel("3"); else if
					 * (levelNumber == "3") { ChangeLevel("1"); }
					 */
					ChangeLevel(levelNumber % 3 + 1);
					// JumpUp resets the turrets look
					jumpUp = true;
			...
			if (!PauseMenuOpen) {
				...
				if (GameSTATE == 1) { // Fuel Turn-Based Mode
					...
					if (turn == 1) { // Player one's turn
						...
						if (keyCode == KeyEvent.VK_W) {
							Mode1 = 1;
							// Tank1.increaseAngle();
							if (Tank1.getAngle() <= 170) {
								// AHMET - reduce number of lines
								/*
								 * checks to see if turret angle has hit max
								 * (max = 172)
								 */
								if (e.isControlDown())
									Tank1.setAngle(Tank1.getAngle() + 4);
								/*
								 * increases the angle by 2
								 */
								else
						...
						{
							Mode1 = 3;
							// removed, no new weapons made yet with the new
							// weapon class
						...
						if ((keyCode == KeyEvent.VK_A) && (curfuel > 0)) {
							/*
							 * This will move the tank if the left key is
							 * pressed and fuel isn't empty.
							 */
							Mode1 = 0;
							// if(Tank1.getMovesLeft()>0) {
							if (Tank1.getTankSprite().getX() >= 3) {
								System.out
										.println("Player 1's tank fuel is at: "
												+ curfuel);
								moving = true;
								Tank1.setMovesLeft(Tank1.getMovesLeft() - 3);
								Tank1.getTankSprite().setX(
										Tank1.getTankSprite().getX() - 3);
								jumpUp = false;
								curfuel -= fueldepletion;
								// AHMET
								/*
								 * Takes fuel away at a constant rate while the
								 * tank is moving.
								 */
							}
							// }
						...
						if ((keyCode == KeyEvent.VK_A) && (curfuel <= 0)) {
							// AHMET
							/*
							 * If the left key is pressed and fuel is empty:
							 * Don't move
							 */
							Mode1 = 0;
							moving = false;
						...
						if ((keyCode == KeyEvent.VK_D) && (curfuel > 0)) {
							...
							if (Tank1.getTankSprite().getX() < 345) {
								...
								Tank1.getTankSprite().setX(
										Tank1.getTankSprite().getX() + 3);
								jumpUp = false;
								curfuel -= fueldepletion; // Takes fuel away at
															// a constant rate
															// while the tank is
															// moving.
							}
						...
						if (keyCode == KeyEvent.VK_F) {
							// AHMET
							/*
							 * When F is pressed, the turn will switch to the
							 * other player
							 */
							...
							// AHMET
							/*
							 * This will make sure the other player has full
							 * fuel at the beginning of his/her turn.
							 */
							...
							if (shotcounter2 > PARALYZED) {
								if (TankShot.getState() == 0) {
									...
									/*
									 * if (TankShot.getState() == 0 &&
									 * SecondaryTankShot.getState() == 0 &&
									 * ThirdTankShot.getState()==0 &&
									 * FourthTankShot.getState()==0) {
									 * 
									 * if(Tank1.getWeapon2()== 0){
									 * TankShot.setState(1);
									 * //soundShot.load("gunshot.mid");
									 * soundShot.play(); } else
									 * if(Tank1.getWeapon2()== 1) {
									 * //soundShot.load("gunshot.mid");
									 * soundShot.play();
									 * SecondaryTankShot.setState(1); } else
									 * if(Tank1.getWeapon2()==2) {
									 * ThirdTankShot.setState(1); } else
									 * if(Tank1.getWeapon2()==3) {
							...
							// CountdownTimer.counterTimer.restart(); //restarts
							// the timer
							// CountdownTimer.counter=HelpScreen.time; //sets
							// the timer to 30
						}
						...
						// if (keyCode == KeyEvent.VK_PERIOD) { // Nuclear
						// Weapon
						...
						if (keyCode == KeyEvent.VK_DOWN) {
							...
							if (Tank2.getAngle() <= 170) {
								...
								if (e.isControlDown())
									Tank2.setAngle(Tank2.getAngle() + 4);
								/*
								 * increases the angle by 2
								 */
								else
						...
						if ((keyCode == KeyEvent.VK_LEFT) && (curfuel > 0)) {
							/*
							 * Makes it so tank can move when fuel isn't empty.
							 */
							...
							if (Tank2.getTankSprite().getX() > 395) {
								...
								Tank2.getTankSprite().setX(
										Tank2.getTankSprite().getX() - 3);
								jumpUp = false;
								curfuel -= fueldepletion;
								/*
								 * Takes fuel away at a constant rate while the
								 ...
								 */
								System.out
										.println("Player 2's tank fuel is at: "
												+ curfuel);
							}
							// }
						}
						...
						if ((keyCode == KeyEvent.VK_RIGHT) && (curfuel > 0)) {
							...
							if (Tank2.getTankSprite().getX() <= 796) {
								...
								// System.out.println("Player 2's tank fuel "
								// + "is at: " + curfuel);
							}
							// }
						}
						...
						if (keyCode == KeyEvent.VK_SHIFT) {
							// curfuel = 110; //Makes sure that the other play
							// will have full fuel at the beginning of his/her
							// turn
							System.out.println(curfuel);
							shotcounter++;
							terrainChange = false;
							...
							if (shotcounter > PARALYZED) {
								...
								if (TankShot.getState() == 0
										&& SecondaryTankShot2.getState() == 0
										&& ThirdTankShot2.getState() == 0
										&& FourthTankShot2.getState() == 0) {
									T2Weapon.fire();
									 ...
									 * //soundShot.load("gunshot.mid");
									 * soundShot.play(); } else
									 * if(Tank2.getWeapon2()== 1) {
									 * //soundShot.load("gunshot.mid");
									 * soundShot.play();
									 * SecondaryTankShot2.setState(1); } else
									 * if(Tank2.getWeapon2()== 2) {
									 * ThirdTankShot2.setState(1); } else
									 * if(Tank2.getWeapon2()== 3) {
				...
				if (keyCode == KeyEvent.VK_ESCAPE) {
					...
					System.out.println("You Pressed: "
							+ KeyEvent.getKeyText(keyCode)); // print out which
																// key the user
																// presses.
		...
		if (titleScreenOpen == 1) {
			if (keyCode == KeyEvent.VK_SPACE) {
				...
				// This needs to be here in order for the tank color changes to
				// take effect
		...
		if (helpScreenOpen == 1) {
			...
			/*
			 * else if (keyCode == KeyEvent.VK_G) { if(GameState==0) {
			 * GameState=1; } else { GameState=0; } }
			 */
	...
	public void DrawMessages(Graphics g) {
		...
		int healthBar1Value = (int) (44 - (Tank1.getHealth() * 4.4));
		/*
		 * Used to determine how much of the health bar is covered
		 */
		int healthBar1Display = (int) (Tank1.getHealth());
		/*
		 * Used to display the value of a player 1's health
		 */
		g.drawImage(healthIcon, 5, 28, null); // health icon
		g.drawImage(healthBar, healthBar1X, healthBar1Y, null); // health bar
		g.setFont(new Font("SansSerif", Font.BOLD, 20));
		/*
		 * Sets the font and size of displayed health number
		 */
		g.setColor(Color.darkGray); // Makes the rectangle that covers health
									// dark gray

		// Draws rectangle that covers Player 1 Health Bar when P1 loses health
		g.drawRect(healthBar.getWidth(null) + (healthBar1X - 4)
				- healthBar1Value, healthBar1Y + 1, healthBar1Value - 4, 20);
		g.fillRect(healthBar.getWidth(null) + (healthBar1X - 4)
				- healthBar1Value, healthBar1Y + 1, healthBar1Value - 4, 20);
		g.setColor(Color.white); // Makes displayed health number white
		g.drawString(Integer.toString(healthBar1Display), (healthBar1X + 5),
				(healthBar1Y + 20)); // Displays the number value of player 1's
										// health
		...
		if ((Mode1 == 0) && (Mode2 == 0)) {
			...
			g.drawImage(fuelBar, barX, barY, null); // Place fuel bar at
													// specified part of the
													// screen
			g.setColor(new Color((fuel - curfuel) / (float) fuel * 0.5f, 0, 0));
			/*
			 * Makes color behind fuel bar black Draws a rectangle to cover what
			 * has been used of the fuel so far.
			 */
		...
		int powerBarValue = 0;
		if (turn == 1)
			powerBarValue = (int) (Tank1.getShotPower() * -111);
		else if (turn == 2)
			powerBarValue = (int) (Tank2.getShotPower() * -111);
		...
		if ((Mode1 == 2) || (Mode2 == 2)) {
			...
			g.setColor(Color.black);
			g.drawRect(barX + powerBarValue, barY + 1, barX - (barX - 110)
					- powerBarValue, 20);
			g.fillRect(barX + powerBarValue, barY + 1, barX - (barX - 110)
					- powerBarValue, 20);
		}
		...
		int healthBar2Value = (int) (44 - (Tank2.getHealth() * 4.4));
		/*
		 * Used to determine how much of player 2's health bar is covered
		 */
		int healthBar2Display = (int) (Tank2.getHealth());
		/*
		 * Used to display the number value of player 2's health
		 */
		g.drawImage(healthIcon, 652, 28, null); // health icon
		g.drawImage(healthBar, healthBar2X, healthBar2Y, null); // health bar
		g.setFont(new Font("SansSerif", Font.BOLD, 20)); // Sets the font and
															// size of the
															// displayed number
		g.setColor(Color.darkGray); // Makes the rectangle that covers health
									// dark gray
		// Draws rectangle that covers Player 2 Health Bar when P2 loses health
		g.drawRect(healthBar.getWidth(null) + (healthBar2X - 1)
				- healthBar2Value, healthBar2Y + 1, healthBar2Value - 1, 20);
		g.fillRect(healthBar.getWidth(null) + (healthBar2X - 1)
				- healthBar2Value, healthBar2Y + 1, healthBar2Value - 1, 20);
		g.setColor(Color.white); // Makes displayed health number white
		g.drawString(Integer.toString(healthBar2Display), (healthBar2X + 5),
				(healthBar2Y + 20)); // Displays the number value of player 2's
										// health
		...
		if (PauseMenuOpen == true) {
			...
			g.setFont(f);
			g.setColor(Color.black);
			g.drawString("t: enter tutorial mode to view the controls.",
					textAlignLeft, 165);
			g.drawString("Spacebar: restart the round", textAlignLeft, 190);
			g.drawString(
					"z: before a game starts, this will change the terrain",
					textAlignLeft, 215);
			g.drawString("Control: hold to accelerate adjusting power/angle",
					textAlignLeft, 240);
			...
			if (WindVar == 2) {
				...
				g.drawString("Wind is randomized after every shot.",
						textAlignLeft, 350);
			} else if (WindVar == 3) {
				g.drawString("Wind is completely randomized.", textAlignLeft,
						350);
			} else {
			...
			g.setFont(f);
			g.setColor(Color.black);
			// g.drawString(playlistnames[HelpScreen.tracknumber],
			// textAlignLeft, 382);
			...
			// ` g.drawString("Press R to restart the game.", textAlignLeft,
			// 425);
	...
	public void updateWeaponandModeIndicator(Graphics g) {
		...
		for (int i = 0; i < 2; i++) { // for each of the two tanks
			...
			if (currentWeapon == 0) {
				...
				strAmmo = Integer.toString(currentAmmo); // Convert int to
															// String
			}
	...
	public void TankFire(Graphics g, int player) {
		...
		if (player == 1) {
			...
			g.drawImage(TankShot.getImage(), Tank1.getTurretAngleX(),
					Tank1.getTurretAngleY(), null);
			TankShot.setState(1);
			TankShot.setX(Tank1.getTurretAngleX());
			TankShot.setY(Tank1.getTurretAngleY());
			p1xveloc = (float) ((-1) * Tank1.getShotPower() * Math.cos(Tank1
					.getAngle() / 57.3));
			p1yveloc = (float) (Tank1.getShotPower() * Math.sin(Tank1
					.getAngle() / 57.3));
			...
			g.drawImage(TankShot2.getImage(), Tank2.getTurretAngleX(),
					Tank2.getTurretAngleY(), null);
			TankShot2.setState(1);
			TankShot2.setX(Tank2.getTurretAngleX());
			TankShot2.setY(Tank2.getTurretAngleY());
			p2xveloc = (float) ((-1) * Tank2.getShotPower() * Math.cos(Tank2
					.getAngle() / 57.3));
			p2yveloc = (float) (Tank2.getShotPower() * Math.sin(Tank2
					.getAngle() / 57.3));
	...
	public void TankFire2(Graphics g, int player) {
		...
		if (player == 1) {
			g.drawImage(SecondaryTankShot.getImage(), Tank1.getTurretAngleX(),
					Tank1.getTurretAngleY(), null);
			...
			SecondaryTankShot.setVelocityX(p1xveloc); // .7 //Player1AngleX
														// Adjustment
			SecondaryTankShot.setVelocityY(p1yveloc); // .3 //POWER
		} else {
			g.drawImage(SecondaryTankShot2.getImage(), Tank2.getTurretAngleX(),
					Tank2.getTurretAngleY(), null);
	...
	public void TankFire3(Graphics g, int player) {
		...
		if (player == 1) {
			g.drawImage(ThirdTankShot.getImage(),
					(int) (Tank2.getTankSprite().getX()), -200, null);
			...
			g.drawImage(ThirdTankShot2.getImage(),
					(int) (Tank1.getTankSprite().getX()), -200, null);
	...
	public void TankFire4(Graphics g, int player) {
		...
		if (player == 1) {
			g.drawImage(FourthTankShot.getImage(),
					(int) (Tank2.getTankSprite().getX()), -100, null);
			FourthTankShot.setState(1);
			...
			g.drawImage(FourthTankShot2.getImage(),
					(int) (Tank1.getTankSprite().getX()), -100, null);
			FourthTankShot2.setState(1);
	...
	public void DrawTerrain(Graphics g) {
		...
		int finalTerrain = Math
				.round((float) ((terrainHeight2 + terrainHeight) / 2) * 100);
		...
		while (basex < 900) {
			if (t > 15) {
				if (t < 90) {
					ang = (t / 57.3);
					tester = Math
							.abs((int) (Math.floor((Math.sin(ang)) * freq)));
					...
					caseChecks[0] = true;
					// tester =
					// Math.abs((int)(Math.floor((Math.sin(ang)*200))));
					// changing 50 to higher # makes higher terrain and lower
					// makes flatter
				}
			}
			if (t > 90) {
			...
			if (levelNumber == 1 || levelNumber == 2 || levelNumber == 5) {
				Color terrainColor = new Color(0x5e3b1f);// Terrain Color
															// Adjustment
			...
			if (levelNumber == 3 || levelNumber == 4) {
				Color terrainColor = new Color(0x5e3b1f);
				// Terrain Color Adjustment
	...
	// I've added the testing lines to it, but as it generates a different
	// terrain I think it should be turned into a new
	// function.

	/*
	 * public void DrawTerrain2(Graphics g) { String enter =
	 * "DrawTerrain has been called!!"; double terrainHeight =
	 * Math.random()+3.42; double terrainHeight2 = Math.random()+5; double abc =
	 * Math.random()*100;
	 * 
	 * 
	 * int freq = Math.round((float)Math.random()*100); int q = 200; int t=15;
	 * int v=0; int tester=342; boolean up = true; double ang; while (basex <
	 * 900) { if (t > 15) { if(t < 90 ) {
	 * 
	 * ang = (t/57.3);//abc tester =
	 * Math.abs((int)(Math.floor((Math.sin(ang))*freq))); //tester =
	 * Math.abs((int)(Math.floor((Math.sin(ang)*200)))); //changing 50 to higher
	 * # makes higher terrain and lower makes flatter caseChecks[0] = true; } }
	 * if (t > 90) { up=false; caseChecks[1] = true; } if(t<15) {up=true;} if
	 * (topy[basex] == null) { //topy[basex]= finalTerrain-tester;//normal
	 * random terrain topy[basex]= q++/2;//decline //topy[basex]=
	 * r--/2;//incline //topy[basex]= 400;//constant caseChecks[5] = true; }
	 * //342 - create a random terrain function from here.
	 * g.setColor(Color.red); g.drawLine(basex,basey,topx,topy[basex]); basex++;
	 * topx++;
	 * 
	 * v++; if (up==true) { caseChecks[2] = true; t++; } if(up==false) {
	 * caseChecks[3] = true; t--; } } while (basex < 2000) {
	 * topy[basex]=342;//342 basex++; caseChecks[4] = true; } basex=0;
	 * basey=900; topx=0; g.setColor(Color.green);
	 * 
	 * caseChecks[6] = true; return; }
	 */

	// *******************************************************

	/*****************************************************
	 * The following blocks of code correspond to the different levels Ryan K
	 *****************************************************
	 */
	public void level() {
		// level 1
	...
	/*
	 * GroundCollision() By Eric Marcarelli, Alex Geden, Ryan Kleckner Starts
	 * and stops the tank from falling, adjusts health, sets rotation angle for
	...
	public void GroundCollision(Sprite tank, int player) {
		...
		int ptx = 0, opp = 0, adj = 0, highest = 1000; // 1000 is an arbitrarly
														// high value that will
														// always be >
														// tank.getY()
		...
		if (topy[(int) tank.getX() + 13] - 13 < (int) tank.getY()) // Allows
																	// tank to
																	// go up
																	// inclines
			tank.setY(topy[(int) tank.getX() + 13] - 13);
		if (tank.getY() < 585) // makes the tank so the tank can't go below the
								// bottom.
		{
			// If the tank's center point is above the ground make the tank
			// fall. Checks to see if it should still be falling
			if (topy[(int) tank.getX() + 13] - 13 > (int) tank.getY())
				falling = true;
		}
		// Possibly add addional cases here that will stop it from falling for
		// realism.
		...
		if (falling && !jumpUp) {
			...
			/*
			 * if (player == 1 && !firstfall ) { if (!moving && !jumpFall)
			 * Tank1.setHealth(Tank1.getHealth()-1);//originally 5 changed to
			 * make for a more balanced game } if (player == 2&& !firstfall2) {
			 * if (!moving && !jumpFall)
			 * Tank2.setHealth(Tank2.getHealth()-1);//originally 5 changed to
			 * make for a more balanced game
			 * 
			 * }
			 */
			...
			/*
			 * Now that it stopped, calculate the rotation angle. This is done
			 * by forming a right triangle and calculating the angle using
			 * arctan(opposite/adjacent).
			 */

			/*
			 * We first determine if it will rotate to the left or right side.
			 * It makes the decision based on the ground at the x location of
			 * each side and the mid pt between middle and side. More cases may
			 * be necessary for additional realism.
			 */

			/*
			 * These loops find the "highest" (actually lowest on the game's y
			 * axis) ground point within the range. It will use this to build
			 * the opposide side in the triangle. The adjacent side is formed
			 * from that pt's x to the center point
			 */

			if ((topy[(int) tank.getX()] - 13 < (int) tank.getY())
					&& (topy[(int) tank.getX() + 9] - 13 < (int) tank.getY())) {
				// Rotating right
			...
			if ((topy[(int) tank.getX() + 38] - 13 < (int) tank.getY())
					&& (topy[(int) tank.getX() + 29] - 14 < (int) tank.getY())) {
				// Rotating left
			...
			// Calculate the angle. Multiplying by the number will let us
			// preserve the angle's sign.
			if (adj != 0)
				angle *= Math.atan((double) opp / adj);
			else
				angle = 0;
			// commented out to try no jumping game play
			// If the tank lands at the bottom of a cliff and there is room,
			// straighten it out
			if (((topy[(int) tank.getX() + 57]) > (int) tank.getY())
					&& (angle > 1.5))
